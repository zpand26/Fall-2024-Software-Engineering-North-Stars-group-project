...
class NutritionGoalPresenter {
  ...
  final cuttingDifferences =
    ...
    final bulkingDifferences = _model.nutrientDifferences(
        event, NutritionGoalModel.bulkingTarget);
    final cuttingDifferences = _model.nutrientDifferences(
        event, NutritionGoalModel.cuttingTarget);

    // Calculate the total absolute difference for each target to find the closest goal
    double bulkingScore = bulkingDifferences.values.map((d) => d.abs()).reduce((
        a, b) => a + b);
    double cuttingScore = cuttingDifferences.values.map((d) => d.abs()).reduce((
        a, b) => a + b);
    double bulkingScore =
    ...
    final withinTarget = differences.entries.where((entry) =>
    entry.value.abs() <= entry.value * 0.1).map((e) => e.key).toList();
    ...
    final aboveTarget = differences.entries
        .where((entry) =>
    entry.value > 0.1 * (closestGoal == 'bulking'
        ? NutritionGoalModel.bulkingTarget[entry.key]!
        : NutritionGoalModel.cuttingTarget[entry.key]!))
        .map((entry) => entry.key)
        .toList();
